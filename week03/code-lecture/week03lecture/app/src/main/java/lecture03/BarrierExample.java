...
class BarrierExample {
	...
	public BarrierExample() {
		...
		// We assign one position of the array to a different thread (variable
		// independence)
		for (int i = 0; i < parties; i++) {
			new SetterClass(i).start();
		}
		System.out.println(Arrays.toString(shared_array));
	}
	...
	public class SetterClass extends Thread {
		...
		public void run() {
			...
			// After this point the array is initialized and it is safe to read it (if you
			// do not comment out the line above)
			System.out.println("Thread index " + index +
					" is printing, shared_array[" + (index + 9) % parties + "]=" + shared_array[(index + 9) % parties]);
		}
	}
}
